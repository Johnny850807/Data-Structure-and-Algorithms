...
import practices.waterball.WbDynamicProgramming;
...
public class DynamicProgrammingTest {
    ...
    @Test
    public void testLCS(){
        int[] s1 = IntStream.range(1, 3001).filter(n -> n % 3 == 0).toArray();
        int[] s2 = IntStream.range(1, 3001).filter(n -> n % 2 == 0).toArray();

        // the lcs would be all multiple numbers of 2 and 3, which can be divided by 6
        int[] lcs = IntStream.range(1, 3001).filter(n -> n % 6 == 0).toArray();
